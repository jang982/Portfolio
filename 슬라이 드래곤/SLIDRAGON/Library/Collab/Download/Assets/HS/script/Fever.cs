...
public class Fever : MonoBehaviour {
    ...
    void FixedUpdate () {
        ...
        {
            GameObject.Find("RayShooter").GetComponent<GameManager>().isFever = true;
        }
        
        if (GameObject.Find("RayShooter").GetComponent<GameManager>().isFever == true && feverLocked == false)
        ...
        if (GameObject.Find("RayShooter").GetComponent<GameManager>().isFever == true && transform.localScale.x <= 0.01f )
            FeverEnd();
    ...
    void FeverEnd()
    {
        fever_count = 0;
        transform.localScale = new Vector3(MAX_FEVER_SCALE * (MIN_FEVER / MAX_FEVER), transform.localScale.y, transform.localScale.z);
        GameObject.Find("RayShooter").GetComponent<GameManager>().isFever = false;
    }
